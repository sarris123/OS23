...
public:
    ...
    Bank& operator=(const Bank& b){
		if(this != &b){
            ...
            bank_mutex_lock(b.bank_read_lock);
            bank_mutex_lock(b.bank_write_lock);*/  
            if(pthread_mutex_lock(&bank_read_lock)){
            perror("Bank error: pthread_mutex_lock failed");
            exit(1);
            }
            if(pthread_mutex_lock(&bank_write_lock)){
            perror("Bank error: pthread_mutex_lock failed");
            exit(1);
            }
            if(pthread_mutex_lock(b.bank_read_lock)){
            perror("Bank error: pthread_mutex_lock failed");
            exit(1);
            }
            if(pthread_mutex_lock(b.bank_write_lock)){
            perror("Bank error: pthread_mutex_lock failed");
            exit(1);
            }
			Bank_accounts = b.Bank_accounts;
            ...
            if(pthread_mutex_unlock(b.bank_read_lock)){
            perror("Bank error: pthread_mutex_lock failed");
            exit(1);
            }
            if(pthread_mutex_unlock(b.bank_write_lock)){
            perror("Bank error: pthread_mutex_lock failed");
            exit(1);
            }
            if(pthread_mutex_unlock(&bank_read_lock)){
            perror("Bank error: pthread_mutex_lock failed");
            exit(1);
            }
            if(pthread_mutex_unlock(&bank_write_lock)){
            perror("Bank error: pthread_mutex_lock failed");
            exit(1);
            }
		}
